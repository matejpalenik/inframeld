...
Default to read-only guidance. Loading this skill grants no editing, grant changes, credential issuance, deployment, or paid-call authority. Resolve code paths from the repository root. Read only the sources relevant to the question, and cite the current rule. The 14-table Access foundation is created by [the initial migration](../../../apps/backend/migrations/versions/0001_initial.py). PostgreSQL integration tests check table presence and selected relational constraints. They do not qualify every Access workflow or document-evidence filtering.
