...
Issue #24's accepted access policy requires **Delete documents** on every group in the Document's nonempty current allowlist, together with current project membership and credential limits, before a human or application may delete that Document. Group managers may explicitly grant this permission within their groups; Add documents and Update documents do not imply it. Check current authority when committing the tombstone. The deletion covers the Document and all its versions, rather than just removing one group's access or changing a future collection selection. [ADR-0005 section 11](ADR-0005-api-enforced-tenancy-and-authorization.md#11-bound-administration-grants-and-recovery) owns the grant rules. This policy still requires implementation and tests.

A human group manager may delete an access group only after no Document or active upload/source configuration references it, as defined in [ADR-0005 section 11](ADR-0005-api-enforced-tenancy-and-authorization.md#11-bound-administration-grants-and-recovery). Resolve dependencies through their normal authorization rules before deletion; never cascade group deletion into document erasure or silent policy/default changes. Deleting the unused group removes its memberships and group-specific grants, and requires no replacement manager or action grantor because the group itself no longer exists.

Deleting an application account requires the separate human-only Delete application account permission on that application, defined in [ADR-0005 section 11](ADR-0005-api-enforced-tenancy-and-authorization.md#11-bound-administration-grants-and-recovery). It permanently stops the account, revokes its keys, and removes its grants and group access, including human administration grants scoped to that account. It does not delete Documents or other project-owned data the application created. No replacement key administrator is needed for a deleted application. Preserve historical attribution under the normal retention/erasure rules rather than reassigning its actions to a human; a retired identity reference must not permit reactivation or authentication.

Deleting a whole project requires the separate human-only **Delete project** permission on that project, as defined in [ADR-0005 section 11](ADR-0005-api-enforced-tenancy-and-authorization.md#11-bound-administration-grants-and-recovery). The project creator receives an explicit initial can-use-and-grant assignment. This authorizes erasure of all project-owned contents, including private documents the actor cannot read, without requiring every contained resource's ordinary deletion permission. It grants no reading or sharing rights. Neither ordinary project nor installation administration implies it. Check current authority and scope at the project tombstone, block affected access/work before cleanup, and preserve out-of-scope data. No replacement manager/grantor is required for resources being deleted with the project; individual deletion within a surviving project keeps its ordinary checks.
...
| **Access-group dependencies** | Reject group deletion while any Document or active upload/source configuration refers to it, including concurrent admissions/configuration changes. Authorized deletion of an unused group removes only its memberships and grants, requires no replacement manager, and never changes document policies or defaults. |
